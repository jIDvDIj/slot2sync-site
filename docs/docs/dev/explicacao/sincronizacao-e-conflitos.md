...
3. **Com manifest, os dois lados mudaram**: conflito real. O Slot2Sync não escolhe sozinho:
   ...
   notifica o usuário. A página do emulador mostra uma seção com os dois lados lado a lado
...
categorias habilitadas (saves/savestates, configurável por emulador; a categoria `config`
existe no schema mas está permanentemente desativada, ver
[Configurações](./configuracoes.md#o-que-saiu-daqui-categorias-de-sync-e-padroes-de-exclusao))
atua nessa conversão, sem tocar o diff nem a resolução de conflito.
