...
O RetroSync é um app instalado (desktop e mobile), incapaz de guardar um client secret de
...
O Android não aceita redirect loopback (sandbox), e custom URI schemes puros (`retrosync://`)
...
(`com.retrosync.app:/oauth2redirect?code=...`). O app registra o listener de deep link antes
...
O `build.rs` lê um `.env` na raiz em build-time e injeta as variáveis `RETROSYNC_*` (shell tem
precedência sobre o arquivo). Sem `RETROSYNC_GOOGLE_CLIENT_ID` configurado, o app sobe
