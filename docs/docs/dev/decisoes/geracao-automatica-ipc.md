...
manual mais alto do que quando a prioridade "média-baixa" abaixo foi atribuída, o que torna
válido reavaliar a urgência à luz do tamanho atual.
...
(mitigável, já que o projeto já usa outros plugins de terceiros, como `tauri-plugin-autostart` e
...
ambos os casos, e `tauri-specta` reduz mais essa fricção por gerar as assinaturas dos
...
Vale adotar uma das duas, mas a escolha entre `ts-rs` (mais simples, só tipos) e `tauri-specta`
