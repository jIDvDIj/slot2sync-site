...
| 1: Desktop completo (Linux/Steam Deck/macOS) | Descoberta e perfis já são cross-platform (`data_dirs` por SO, incluindo caminhos Flatpak de Steam Deck/EmuDeck). Builds `.dmg`/`.deb`/AppImage já rodam via runners hospedados do GitHub Actions (`macos-latest`/`ubuntu-22.04` em `release.yml`), sem depender de máquina própria. O que falta é notarização/assinatura Apple e o empacotamento Flatpak específico (o Tauri não gera Flatpak nativamente). | 🟡 Em andamento |
...
| 7: Gatilhos e UI mobile | `resume`/`pause` como gatilhos de sync; `AddEmulatorDialog`/`SettingsPage` e outros componentes adaptados por plataforma (`usePlatform`/`isMobile`). | ✅ Concluída |
